...
CONFIDENCE_THRESHOLD = 0.7  # Lowered to match the TFT model's threshold
...
MAX_KELLY_FRACTION = (
    0.05  # Maximum fraction of bankroll to risk (conservative approach)
)
...
def calculate_kelly_fraction(recent_trades):
    ...
    if len(recent_trades) < MIN_TRADES_FOR_KELLY:
        ...
        )  # Use conservative sizing when insufficient trade history
...
def simulate_trading(
    ...
    for i in range(len(df)):
        ...
        if use_kelly:
            ...
            confidence_adjusted_kelly = kelly_fraction * (
            ...
            for conf_threshold, trade_size in sorted(
                CONFIDENCE_TRADE_SIZES.items(), reverse=True
            ):
        ...
        trades.append(
            {
                ...
                "Profit": profit,
                "Confidence": (
                    df["Confidence"].iloc[-1] if "Confidence" in df.columns else 1.0
                ),
                "Kelly Fraction": kelly_fraction if use_kelly else None,
            }
        )
    ...
    recent_win_rate = (
        (
            sum(1 for trade in recent_trade_profits if trade > 0)
            / len(recent_trade_profits)
        )
        if recent_trade_profits and use_kelly
        else 0
    )
...
def main():
    ...
    if kelly_roi > fixed_roi:
        print(
            f"Kelly Criterion outperformed Fixed Sizing by {kelly_roi - fixed_roi:.2f}% ROI"
        )
    elif fixed_roi > kelly_roi:
        print(
            f"Fixed Sizing outperformed Kelly Criterion by {fixed_roi - kelly_roi:.2f}% ROI"
        )
    else:
